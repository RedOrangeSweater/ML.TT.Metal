...
- sub‑SDLC: `docs/sdlc/NN_<folder>/` (например `01_GameIdeas`, `02_ReleaseGates`)

### Input

- **folder** — имя подпроцесса (без префикса `NN_`, например `game_ideas`, `release_gates`). Папка будет создана как `docs/sdlc/NN_<folder>/`.
...
- **domainTags** — опционально: теги для реестра (например `ideas`, `release`).
...
- Примеры: `docs/sdlc/01_GameIdeas/`, `docs/sdlc/00_Main/`.
